  ...
  where T : class
{
  ...
  public SnowflakeCommand<T> OrderByAsc<TGroupBy>(Expression<Func<T, TGroupBy>> predicate) => this.OrderBy(predicate, "ASC");
  ...
  /// <returns>The snowflake command.</returns>
  /// <exception cref="InvalidOperationException">Command already has an order by clause!</exception>
  public SnowflakeCommand<T> OrderByDesc<TGroupBy>(Expression<Func<T, TGroupBy>> predicate) => this.OrderBy(predicate, "DESC");
  ...
  /// <returns>The snowflake command.</returns>
  /// <exception cref="InvalidOperationException">Command already has an order by clause!</exception>
  protected SnowflakeCommand<T> OrderBy<TGroupBy>(Expression<Func<T, TGroupBy>> predicate, string direction)
  {
    ...
    foreach (var property in this.Properties)
    {
      ...
      orderByBody = orderByBody.Replace($"{property.Key.Name}", $"{propertyTableAlias}.{propertyName}");
    }
  ...
  public SnowflakeCommand<T> OrderBy(string orderBy)
  {
    ...
    this.SqlBuilder.Append($" {(orderBy.Trim().StartsWith("ORDER BY", ignoreCase: true, null) ? orderBy.Trim() : $"ORDER BY {orderBy.Trim()}")}");
